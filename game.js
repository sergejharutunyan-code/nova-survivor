...
const WEAPONS = {
  ...
  spread:  { name:'Scatter Cannon', icon:'scatter', desc:'Sprays a wide cone of pellets in front of you. Crowd shredder.',
  ...
  nova:    { name:'Nova Pulse', icon:'nova', desc:'Periodic shockwave damaging everything around you.',
             cd:1.8, dmg:30, color:'#ff4d6d', max:7,
...
// weighted pool — tougher enemies are introduced wave by wave and grow more common
function enemyTypeFor(w) {
  const pool = [['grunt',10],['swarm',8]];
  if (w >= 2)  pool.push(['tank',     4 + w*0.40]);
  if (w >= 3)  pool.push(['dasher',   3 + w*0.50]);
  if (w >= 4)  pool.push(['shooter',  3 + w*0.40]);
  if (w >= 5)  pool.push(['splitter', 3 + w*0.50]);
  if (w >= 7)  pool.push(['bomber',   3 + w*0.50]);
  if (w >= 9)  pool.push(['healer',   2 + w*0.30]);
  if (w >= 11) pool.push(['brute',    2 + w*0.50]);
...
const NORMAL_PER_ULTIMATE = 2;   // after this many normal bosses, an ultimate event fires
const ULTRA_BOSSES = [
...
function spawnUltra(def, hs, angle) {
  ...
  const e = {
    ...
    hp: def.hp*hs, maxHp: def.hp*hs,
    spd: def.spd, dmg: def.dmg * dmgScale(G.wave),
    xp:60, coin:30, boss:true,
...
function killEnemy(e) {
  e.dead = true;
  G.kills++;
  const gemN = e.ultra ? 28 : e.boss ? 14 : e.elite ? 5 : 1;
  ...
  const coinN = e.ultra ? 20 : e.boss ? 12 : e.elite ? 3 : 1;
  if (e.boss || e.elite || Math.random() < e.coin) {
    for (let i=0;i<coinN;i++)
      G.coins.push({ x:e.x+rand(-e.r,e.r), y:e.y+rand(-e.r,e.r), vx:rand(-50,50), vy:rand(-50,50), v: e.ultra?3:e.boss?2:1, r:6 });
  }
  ...
  if (e.boss) {
    ...
    const bonus = Math.round((e.ultra ? 20 : 8) * G.player.coinMult);
    G.runCoins += bonus;
...
function fireWeapon(w, def, p) {
  ...
  switch (w.key) {
    case 'blaster': {                               // fan of bolts ahead, with semi-autoaim
      ...
      for (let i = 0; i < count; i++) {
        ...
        fireBullet(p.x, p.y, aim+off, def, ndmg,
          { pierce: w.evolved ? 999 : Math.floor((lvl-1)/3), r: w.evolved?8:6, life: 1.4*p.rangeMult, speedMul: w.evolved?1.3:1 });
      }
      SFX.shoot(); break;
    }
    case 'spread': {                                // wide cone in front
      const pellets = projCount(w, p);
      const arc = 0.95;
      for (let i = 0; i < pellets; i++) {
        const off = pellets > 1 ? (i/(pellets-1) - 0.5) * arc : 0;
        // evolved: pellets detonate on impact
        fireBullet(p.x, p.y, p.face+off, def, ndmg, { r:5, life:0.75*p.rangeMult, blast: w.evolved?70:0 });
      }
      SFX.shoot(); break;
    }
    case 'missile': {                               // fan out in front, then home
      const n = projCount(w, p) + (w.evolved ? 2 : 0);
      for (let i = 0; i < n; i++) {
        const a = p.face + (i - (n-1)/2) * 0.22;
        fireBullet(p.x, p.y, a, def, ndmg*1.1, { homing:true, r:7, life:2.4*p.rangeMult, pierce:1, emoji:'missile', blast: w.evolved?90:0 });
      }
      SFX.shoot(); break;
    }
    case 'nova': {
      const radius = (110 + lvl*22) * (w.evolved ? 1.4 : 1);
    ...
    case 'laser': {
      ...
      const targets = w.evolved ? nearestEnemies(p.x, p.y, 3) : [nearestEnemy(p.x, p.y)].filter(Boolean);
      if (!targets.length) { w._cd = 0.2; break; }
      for (const target of targets) {
        const ang = Math.atan2(target.y-p.y, target.x-p.x);
        const len = 1200, ex = p.x+Math.cos(ang)*len, ey = p.y+Math.sin(ang)*len;
...
function updateOrbs(dt) {
  ...
  const radius = 70 + w.level*6 + (w.evolved ? 40 : 0);
...
function update(dt) {
  ...
  for (const b of G.bullets) {
    if (b.homing) {
      const t = nearestEnemy(b.x, b.y, 360);
      if (t) {
    ...
    for (const e of G.enemies) {
      ...
      if (dist2(b.x,b.y,e.x,e.y) < (b.r+e.r)*(b.r+e.r)) {
        const kx = b.vx*0.15, ky = b.vy*0.15;
