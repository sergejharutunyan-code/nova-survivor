...
const WEAPONS = {
  blaster: { name:'Plasma Blaster', icon:'gun', desc:'Fires a fan of bolts ahead — snaps onto enemies in your sights. More bolts as it levels.',
             cd:0.55, dmg:12, speed:520, pierce:0, color:'#3df0ff', max:7, normal:true, kb:1,
  ...
  spread:  { name:'Scatter Cannon', icon:'scatter', desc:'Point-blank shotgun — a wide spray of pellets that knocks enemies back. Crowd shredder.',
             cd:0.95, dmg:7, speed:460, pierce:0, color:'#ffcf3a', max:7, normal:true, kb:1.3,
  ...
  missile: { name:'Homing Swarm', icon:'rocket', desc:'Launches seeking micro-missiles that fan out, then chase enemies down.',
             cd:1.15, dmg:18, speed:300, pierce:0, color:'#ff8a5b', max:7, normal:true, kb:0,
  ...
  orbit:   { name:'Aegis Orbs', icon:'orbit', desc:'Energy orbs circle you, smashing anything they touch.',
             cd:0, dmg:18, color:'#8a5bff', max:7, kb:0.2,
  ...
  nova:    { name:'Nova Pulse', icon:'nova', desc:'Periodic shockwave damaging everything around you.',
             cd:2.2, dmg:30, color:'#ff4d6d', max:7, kb:2,
  ...
  laser:   { name:'Rail Beam', icon:'beam', desc:'Instant piercing beam through a whole line of enemies.',
             cd:1.3, dmg:34, color:'#34e07a', max:7, kb:0.1,
  ...
  shield:  { name:'Aegis Shield', icon:'shield', desc:'A barrier that blocks one incoming hit (projectile or contact), then recharges over time — faster with Fire Rate.',
             cd:0, dmg:0, color:'#39e3ff', max:7, kb:0,
             evolve:{ name:'Riposte Field', stat:'regen', statName:'Nanobots', dmgMul:1,
                      desc:'A blocked hit fires a retaliating arc back where the shot came from — damaging foes, 25% chance to delete projectiles in its path.' } },
...
const KB_BASE = 80;   // reference knockback impulse; each weapon's `kb` scales it (then ×player.knockback ÷ enemy mass)
...
function newPlayer() {
  ...
  const player = {
    ...
    crit: s.crit, critMult: s.critMult, projBonus: 0, projMult: 1, rangeMult: 1, knockback: 0.1, shield: 0, shieldMax: 0, shieldCD: 0, statLvl: {},
...
function killEnemy(e) {
  ...
  if (e.bomb && !e._exploded) {
    ...
    if (dist2(e.x, e.y, G.player.x, G.player.y) < R*R) hurtPlayer(e.dmg * 2.5, Math.atan2(e.y - G.player.y, e.x - G.player.x));
  }
...
function fireBullet(x, y, ang, def, dmgBase, extra={}) {
  ...
  G.bullets.push({
    ...
    blast: extra.blast||0, kb: (def.kb != null ? def.kb : 1), twirl: extra.twirl || 0, spin: extra.spin || 0,
  });
}
function updateWeapons(dt) {
  ...
  for (const w of p.weapons) {
    ...
    if (w.key === 'orbit' || w.key === 'shield') continue; // continuous / passive
  ...
  switch (w.key) {
    ...
    case 'spread': {                                // shotgun: a wide, jittery spray with real punch
      ...
      for (let i = 0; i < pellets; i++) {
        ...
        fireBullet(p.x, p.y, p.face+off, def, ndmg, { r:5, life: firingRange(p)/def.speed, blast: w.evolved?70:0 });
      }
      SFX.shoot(); break;
    }
    case 'missile': {                               // burst out to the sides & back, twirl, then hunt
      const want = projCount(w, p) + (w.evolved ? 2 : 0);
      const cap = 12 + w.level*4 + (w.evolved ? 12 : 0) + p.projBonus*4;   // max missiles out at once (doubled)
    ...
    case 'nova': {
      const base = firingRange(p) * 0.75;   // 3/4 of the blaster's range
      let radius = base;
      // Solar Flare: no permanent widen — instead every 7th pulse erupts at 3× the distance
      if (w.evolved) { w._pulse = (w._pulse || 0) + 1; if (w._pulse % 7 === 0) radius = base * 3; }
      ...
      for (const e of G.enemies) {
        ...
        if (dist2(p.x,p.y,e.x,e.y) < radius*radius) {
          ...
          dealDamage(e, d, Math.cos(a)*KB_BASE*def.kb, Math.sin(a)*KB_BASE*def.kb, crit);
      ...
      for (const target of targets) {
        ...
        for (const e of G.enemies) {
          ...
          if (pointSegDist2(e.x, e.y, p.x, p.y, ex, ey) < (thick+e.r)*(thick+e.r)) {
            ...
            dealDamage(e, d, Math.cos(ang)*KB_BASE*def.kb, Math.sin(ang)*KB_BASE*def.kb, crit);
          }
        }
      }
      SFX.shoot(); break;
...
function updateOrbs(dt) {
  ...
  const radius = firingRange(p) * 0.5 * (w.evolved ? (2 + Math.sin(G.time * 1.2)) : 1);   // base half-range; Halo of Ruin breathes 1×→3×
  ...
  for (const o of G.orbs) {
    o.a += dt * (w.evolved ? 3.4 : 2.4) * p.fireRateMult;   // Fire Rate spins the orbs faster
  ...
  for (const e of G.enemies) {
    ...
    for (const o of G.orbs) {
      if (dist2(o.x,o.y,e.x,e.y) < (18+e.r)*(18+e.r)) {
        const a = Math.atan2(e.y-p.y, e.x-p.x);
        const { dmg:d, crit } = rollDamage(dmg);
        dealDamage(e, d, Math.cos(a)*KB_BASE*WEAPONS.orbit.kb, Math.sin(a)*KB_BASE*WEAPONS.orbit.kb, crit);
        e.orbCD = 0.28; break;
      }
    }
  }
}
function shieldRegenInterval(p) { return Math.max(2.5, 6 / (p.fireRateMult || 1)); }   // capped: never recharges too fast
function updateShield(dt) {
  const p = G.player, w = getW(p, 'shield');
  if (!w) { p.shieldMax = 0; p.shield = 0; return; }
  p.shieldMax = 1 + Math.floor(w.level / 2) + (w.evolved ? 2 : 0);   // L1:1 … L7:4 (+2 evolved)
  if (p.shield == null || p.shield > p.shieldMax) p.shield = p.shieldMax;
  if (p.shield < p.shieldMax) {
    p.shieldCD = (p.shieldCD || shieldRegenInterval(p)) - dt;
    if (p.shieldCD <= 0) { p.shield++; p.shieldCD = shieldRegenInterval(p); ring(p.x, p.y, p.r + 14, '#39e3ff'); }
  } else p.shieldCD = shieldRegenInterval(p);
}
function shieldArc(p, ang) {                                  // evolved Riposte Field: retaliating arc toward the shot's source
  const len = firingRange(p) * 1.5;
  const ex = p.x + Math.cos(ang)*len, ey = p.y + Math.sin(ang)*len;
  G.beams.push({ x1:p.x, y1:p.y, x2:ex, y2:ey, life:0.16, color:'#39e3ff' });
  const admg = 60 * p.damageMult;
  for (const e of G.enemies) {
    if (e.dead) continue;
    if (pointSegDist2(e.x, e.y, p.x, p.y, ex, ey) < (20 + e.r)*(20 + e.r)) {
      const { dmg:d, crit } = rollDamage(admg); dealDamage(e, d, 0, 0, crit);
    }
  }
  for (const b of G.ebullets) {                              // 25% chance to delete enemy shots in the arc's path
    if (pointSegDist2(b.x, b.y, p.x, p.y, ex, ey) < (b.r + 24)*(b.r + 24) && Math.random() < 0.25) b.dead = true;
  }
}
...
function update(dt) {
  ...
  for (const e of G.enemies) {
    ...
    if (dist2(e.x,e.y,p.x,p.y) < (e.r+p.r)*(e.r+p.r)) {
      ...
      hurtPlayer(e.dmg*dt*1.4 * cm, Math.atan2(e.y - p.y, e.x - p.x));
    }
  }
  ...
  for (const b of G.bullets) {
    ...
    for (const e of G.enemies) {
      ...
      if (dist2(b.x,b.y,e.x,e.y) < (b.r+e.r)*(b.r+e.r)) {
        const _m = Math.hypot(b.vx, b.vy) || 1, _ki = KB_BASE * (b.kb || 0), kx = b.vx/_m*_ki, ky = b.vy/_m*_ki;
  ...
  for (const b of G.ebullets) {
    b.x += b.vx*dt; b.y += b.vy*dt; b.life -= dt;
    if (b.life <= 0) { b.dead = true; continue; }
    if (dist2(b.x,b.y,p.x,p.y) < (b.r+p.r)*(b.r+p.r)) { hurtPlayer(b.dmg, Math.atan2(-b.vy, -b.vx)); b.dead = true; }
  }
...
function hurtPlayer(amount, srcAng) {
  const p = G.player;
  if (p.invuln > 0) return;
  // Aegis Shield: a charge fully blocks the hit (projectile or contact), then a brief grace window
  if (p.shield > 0) {
    p.shield--; p.shieldCD = shieldRegenInterval(p); p.invuln = 0.35;
    ring(p.x, p.y, p.r + 16, '#39e3ff'); SFX.hit();
    const sw = getW(p, 'shield');
    if (sw && sw.evolved && srcAng != null) shieldArc(p, srcAng);
    return;
  }
  p.hp -= amount;
...
function render() {
  ...
  if (p.shield > 0) {                                    // shield bubble — brighter with more charges
    ctx.save();
    ctx.strokeStyle = 'rgba(57,227,255,' + Math.min(0.85, 0.32 + p.shield*0.16) + ')';
    ctx.lineWidth = 2.5; ctx.beginPath(); ctx.arc(px, py, p.r + 13, 0, TAU); ctx.stroke();
    ctx.restore();
  }
...
function renderStats(mode='base') {
  ...
  if (live) {
    ...
    if (pl.shieldMax > 0) comps.push(['shield','Shield',(pl.shield||0)+'/'+pl.shieldMax,'weapon']);
...
function evCount()  { return ['blaster','spread','missile','orbit','nova','laser'].filter(k => save.mastery.evolved.includes(k)).length; }
